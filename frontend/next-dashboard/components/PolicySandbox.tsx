...
export default function PolicySandbox({ onUpdateImpact }: { onUpdateImpact: (impact: number) => void }) {
    ...
    const togglePolicy = (id: string) => {
        ...
        setTimeout(() => {
            ...
            const totalImpact = updatedPolicies
                .filter(p => p.active)
    ...
    return (
        <div className="flex flex-col gap-4 p-5 bg-white/5 border border-white/10 rounded-2xl relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-2">
                ...
                {isQuerying && <span className="text-[9px] text-blue-400 animate-pulse">RUNNING QUERY...</span>}
            </div>
                    ...
                    >
                        ...
                        <div className="flex flex-col items-end">
                            <span className={`text-[10px] font-mono font-bold ${policy.active ? 'text-neon-emerald' : 'text-white/20'}`}>
                                {policy.active ? `-${policy.impact_co2}%` : '0%'}
                            </span>
                            {/* Checkbox visual */}
